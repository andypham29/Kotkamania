...
class NhlPlayerStatFacade:
    ...
    def get_all_players_stats(self, season_id: int, limit: int = 500, batch: int = 100) -> List[PlayerStat]:
        """
        Get comprehensive statistics for all players in a season using paging.

        Args:
            season_id: NHL season ID
            limit: Maximum number of players to retrieve (default 500)
            batch: Page size for each API request (default 100)

        Returns:
            List of PlayerStat objects
        """
        # Fetch paged data from each service up to `limit` items
        realtime_players = self._fetch_paged(self.realtime_service.getAllPlayers, season_id, total=limit, batch=batch)
        summary_players = self._fetch_paged(self.summary_service.getAllPlayers, season_id, total=limit, batch=batch)
        timeonice_players = self._fetch_paged(self.timeonice_service.getAllPlayers, season_id, total=limit, batch=batch)

        # Create a map of player_id to data for each service
    ...
    def get_forwards_stats(self, season_id: int, limit: int = 500, batch: int = 100) -> List[PlayerStat]:
        """
        Get comprehensive statistics for forwards in a season.
        """
        realtime_players = self._fetch_paged(self.realtime_service.getForwards, season_id, total=limit, batch=batch)
        summary_players = self._fetch_paged(self.summary_service.getForwards, season_id, total=limit, batch=batch)
        timeonice_players = self._fetch_paged(self.timeonice_service.getForwards, season_id, total=limit, batch=batch)

        return self._combine_player_data(realtime_players, summary_players, timeonice_players, season_id)

    def get_defensemen_stats(self, season_id: int, limit: int = 500, batch: int = 100) -> List[PlayerStat]:
        """
        Get comprehensive statistics for defensemen in a season.
        """
        realtime_players = self._fetch_paged(self.realtime_service.getDefensemen, season_id, total=limit, batch=batch)
        summary_players = self._fetch_paged(self.summary_service.getDefensemen, season_id, total=limit, batch=batch)
        timeonice_players = self._fetch_paged(self.timeonice_service.getDefensemen, season_id, total=limit, batch=batch)
...
if __name__ == '__main__':
    ...
    season_id = 20252026
    # Use the facade method which handles paging automatically (loops to fetch 500 players)
    stats = facade.get_all_players_stats(season_id, limit=500, batch=100)

    # print each PlayerStat in a readable JSON format, one per line
    for s in stats:
