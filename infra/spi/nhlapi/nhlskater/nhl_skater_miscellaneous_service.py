...
from infra.spi.nhlapi.nhlskater.model.nhl_skater_miscellaneous import SkaterMiscellaneous
from server.commons.helper.nhl_season_converter import NhlYearConverter

_MISCELLANEOUS_FIELDS = {f.name for f in fields(SkaterMiscellaneous)}

_SORT = (
    "%5B%7B%22property%22:%22hits%22,%22direction%22:%22DESC%22%7D,"
    "%7B%22property%22:%22playerId%22,%22direction%22:%22ASC%22%7D%5D"
)


class NHLSkaterMiscellaneousService:
    ...
    def __to_model(self, row: dict) -> SkaterMiscellaneous:
        return SkaterMiscellaneous(**{k: v for k, v in row.items() if k in _MISCELLANEOUS_FIELDS})


if __name__ == '__main__':
    import json
    players = NHLSkaterMiscellaneousService().getAllPlayers(end="5", seasonId=20252026)
