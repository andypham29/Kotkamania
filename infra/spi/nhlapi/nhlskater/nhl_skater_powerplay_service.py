...
from infra.spi.nhlapi.nhlskater.model.nhl_skater_powerplay import SkaterPowerPlay
from server.commons.helper.nhl_season_converter import NhlYearConverter

_POWERPLAY_FIELDS = {f.name for f in fields(SkaterPowerPlay)}

_SORT = (
    "%5B%7B%22property%22:%22ppTimeOnIce%22,%22direction%22:%22DESC%22%7D,"
    "%7B%22property%22:%22playerId%22,%22direction%22:%22ASC%22%7D%5D"
)


class NHLSkaterPowerPlayService:
    ...
    def __to_model(self, row: dict) -> SkaterPowerPlay:
        return SkaterPowerPlay(**{k: v for k, v in row.items() if k in _POWERPLAY_FIELDS})


if __name__ == '__main__':
    import json
    players = NHLSkaterPowerPlayService().getAllPlayers(end="5", seasonId=20252026)
