...
from infra.spi.nhlapi.nhlskater.model.nhl_skater_shotattemptcount import SkaterShotAttemptCount
from server.commons.helper.nhl_season_converter import NhlYearConverter

_SHOTATTEMPTCOUNT_FIELDS = {f.name for f in fields(SkaterShotAttemptCount)}

_SORT = (
    "%5B%7B%22property%22:%22satFor%22,%22direction%22:%22DESC%22%7D,"
    "%7B%22property%22:%22playerId%22,%22direction%22:%22ASC%22%7D%5D"
)


class NHLSkaterShotAttemptCountService:
    ...
    def __get_url_team_paging(self, franchiseId, start, limit, seasonId):
        return (
            f"https://api.nhle.com/stats/rest/en/skater/summaryshooting"
    ...
    def __to_model(self, row: dict) -> SkaterShotAttemptCount:
        return SkaterShotAttemptCount(**{k: v for k, v in row.items() if k in _SHOTATTEMPTCOUNT_FIELDS})


if __name__ == '__main__':
    import json
    players = NHLSkaterShotAttemptCountService().getAllPlayers(end="5", seasonId=20252026)
