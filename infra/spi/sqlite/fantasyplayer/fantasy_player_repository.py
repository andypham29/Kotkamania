...
class FantasyPlayerRepository:
    ...
    def deleteAllFantasySkater(self):
        """Delete fantasy skater by ID"""
        session = self._get_session()
        try:
            session.query(FantasyNhlPlayerORM).delete()
            session.commit()
        finally:
            session.close()
    ...
    def __convert_to_model_with_display_stat(orm_row, stat_row):
        ...
        display_stat = None
        if stat_row:
            display_stat = SqliteDisplayStat(
        ...
        return SqliteFantasyNhlPlayer(
            ...
            stat=display_stat or SqliteDisplayStat()
        )
