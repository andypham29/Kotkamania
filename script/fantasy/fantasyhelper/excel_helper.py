...
class ExcelHelper:
    ...
    def write_players_to_excel(self, sheet='forward', players=[]):
        ...
        for a in range(len(players)):
            ...
            worksheet.write(a + 1, 5, f"{players[a].assists} ({round(players[a].assists / players[a].games * 82, 2)})")
            worksheet.write(a + 1, 6, f"{players[a].points} ({round(players[a].points / players[a].games * 82, 2)})")
